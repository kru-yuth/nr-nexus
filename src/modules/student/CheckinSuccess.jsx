    ...
    return (
        <div className="min-h-screen bg-[#1a5c38] flex flex-col items-center justify-center p-6 text-center">
            <div className="bg-white rounded-[4rem] p-12 shadow-2xl max-w-md w-full animate-in zoom-in-95 duration-500">
                ...
                <h1 className="text-4xl font-black text-slate-900 mb-2 uppercase italic tracking-tight">{t('checkin_success')}</h1>
                <p className="text-slate-400 font-bold text-sm uppercase tracking-widest mb-10">{t('checkin_success_desc')}</p>

                <div className="bg-slate-50 rounded-3xl p-8 mb-8 space-y-6">
                    <div className="flex justify-between items-center text-left">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('late_status')}</span>
                        <span className="text-sm font-black text-rose-600 uppercase">
                            {result?.status === 'late_minor' ? t('late_minor') : result?.status === 'warning' ? t('late_warning') : t('late_major')}
                        </span>
                    </div>
                    <div className="h-px bg-slate-200" />
                    <div className="flex justify-between items-end">
                        <div className="text-left">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">{t('points_deducted')}</span>
                            <div className="text-2xl font-black text-rose-500">-{result?.deduction ?? 0}</div>
                        </div>
                        <div className="text-right">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">{t('points_remaining')}</span>
                            <div className="text-3xl font-black text-slate-900 tabular-nums">
                                {result?.newScore ?? 100}<span className="text-sm text-slate-300 ml-1">/100</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-10">
                    <div className="h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                        <div 
                            className={`h-full transition-all duration-1000 ${(result?.newScore ?? 100) > 80 ? 'bg-emerald-500' : (result?.newScore ?? 100) > 50 ? 'bg-amber-500' : 'bg-rose-500'}`}
                            style={{ width: `${result?.newScore ?? 100}%` }}
                        />
                    </div>
                </div>
            ...
            <p className="mt-12 text-emerald-100/40 text-[10px] font-bold uppercase tracking-[0.3em] flex items-center gap-3">
                <CheckCircle2 size={14} /> {t('discipline_staff_notified') || "ครูฝ่ายปกครองได้รับแจ้งเตือนแล้ว"}
            </p>
        </div>
    );
};
